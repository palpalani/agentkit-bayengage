...
export const listTemplatesTool = tool({
  ...
  async execute(input) {
    ...
    const response = await client.get<{ templates: Template[]; total: number }>(endpoint);
...
export const getTemplateTool = tool({
  ...
  async execute(input) {
    ...
    const response = await client.get<Template>(`/templates/${input.templateId}`);
...
export const createTemplateTool = tool({
  ...
  async execute(input) {
    ...
    const response = await client.post<Template>('/templates', {
      name: input.name,
...
export const updateTemplateTool = tool({
  ...
  async execute(input) {
    ...
    if (input.htmlContent) updateData.html_content = input.htmlContent;
    if (input.category) updateData.category = input.category;

    const response = await client.patch<Template>(`/templates/${input.templateId}`, updateData);
