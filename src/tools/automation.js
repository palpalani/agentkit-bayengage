...
export const createDripCampaignTool = tool({
    ...
    async execute(input) {
        ...
        const response = await client.post('/automation/drip-campaigns', {
            name: input.name,
...
export const activateDripCampaignTool = tool({
    ...
    async execute(input) {
        const client = getBayEngageClient();
        const response = await client.patch(`/automation/drip-campaigns/${input.dripCampaignId}/activate`, { status: 'active' });
        if (response.status === 'error') {
            return {
                success: false,
...
export const createABTestCampaignTool = tool({
    ...
    async execute(input) {
        ...
        const client = getBayEngageClient();
        const response = await client.post('/campaigns/ab-tests', {
            name: input.name,
...
export const startABTestTool = tool({
    ...
    async execute(input) {
        ...
        const client = getBayEngageClient();
        const response = await client.patch(`/campaigns/ab-tests/${input.abTestId}/start`, { status: 'testing' });
        if (response.status === 'error') {
            return {
                success: false,
...
export const createNewsletterTool = tool({
    ...
    async execute(input) {
        const client = getBayEngageClient();
        const response = await client.post('/automation/newsletters', {
            name: input.name,
...
export const pauseNewsletterTool = tool({
    ...
    async execute(input) {
        const client = getBayEngageClient();
        const response = await client.patch(`/automation/newsletters/${input.newsletterId}`, { status: 'paused' });
        if (response.status === 'error') {
            return {
                success: false,
