...
export const listSegmentsTool = tool({
    ...
    async execute(input) {
        ...
        const response = await client.get(`/segments?limit=${input.limit}&offset=${input.offset}`);
...
export const sendCampaignTool = tool({
    ...
    parameters: z.object({
        ...
        scheduledAt: z
            .string()
            .optional()
            .describe('ISO 8601 timestamp to schedule send (optional, sends immediately if not provided)'),
        confirmSend: z
            .boolean()
            .default(false)
    ...
    async execute(input) {
        ...
        const response = await client.patch(`/campaigns/${input.campaignId}/send`, payload);
        if (response.status === 'error') {
            return {
                success: false,
...
export const getCampaignStatsTool = tool({
    ...
    async execute(input) {
        const client = getBayEngageClient();
        const response = await client.get(`/campaigns/${input.campaignId}/stats`);
        if (response.status === 'error') {
            return {
                success: false,
