...
async function example2_specializedAgents() {
  ...
  const campaignResponse = await client.chat(
  ...
  // Data Analyst Agent
  const analyticsResponse = await client.chat(
...
async function example4_contactManagement() {
  ...
  // Create contact
  const createResponse = await client.chat(
...
async function example5_campaignWorkflow() {
  ...
  // Create campaign
  const createResp = await client.chat(
    'Create campaign "Monthly Newsletter" with subject "January Updates" using template newsletter_v1 for segment subscribers',
    'campaign_manager'
  );
...
async function example8_dripCampaign() {
  ...
  const response = await client.chat(
    'Create welcome drip campaign: Day 0 welcome email, Day 2 product tour, Day 5 special offer'
  );
...
async function example9_abTesting() {
  ...
  const response = await client.chat(
    'Create A/B test for subject lines: "Last Chance" vs "Don\'t Miss Out", test 20% of audience',
    'campaign_manager'
  );
